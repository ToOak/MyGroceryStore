...
public class DataConverter {
	...
	/**
	 * 
	 * @param addPos will create strings len = ori.length + addPos
	 * @param ori
	 * @param PaddingLeft whether inserts tab before string or not
	...
	/**
	 * when string contains Chinese, we should get the byte length separately<br>
	...
	public static String joinStrings(String firstStr, String secondStr) {
		if(Validator.isEffective(firstStr) == false
				&& Validator.isEffective(secondStr) == false)
			return "";
		if(Validator.isEffective(firstStr) == true
				&& Validator.isEffective(secondStr) == false)
			return firstStr;
		if(Validator.isEffective(firstStr) == false
				&& Validator.isEffective(secondStr) == true)
			return secondStr; 
	...
	/**
	 * @param lng 123.456789555
	 * @param lat 98.765432555
	 * @return 123459876
	 */
	...
	/**
	 * 
	 * @param strIntOrDouble
	 * @return null or empty return 0
	 */
	public static int parseIntOrDouble(String strIntOrDouble) {
		try {
			return Integer.parseInt(strIntOrDouble);
		} catch (Exception e1) {
			try {
				return (int) Double.parseDouble(strIntOrDouble);
			} catch (Exception e2) {
				return 0;
			}
		}
	...
	/**
	 * 
	 * @param str which need be masked
	 * @param masker like "*", "#" etc
	 * @param startPos start from zero
	 * @param masklen the length fill with masker
	...
	public static String convert(String str, int index) {
		...
		try {
			...
			int len = a.length;
			int len2 = Constant.gchar.length();
			for (int i = 0; i < len; i++) {
				a[i] = (char) (a[i] ^ Constant.gchar.charAt((i + index) % len2));
			}
		...
		for (int i = 0; i < len; i++) {
			for (pos = 0; pos < size;) {
				...
				if (index >= 0) {
					if (index == 0)
						return index + keys[i].length();
					if (("" + str.charAt(index - 1)).replaceAll(spliter, "")
							.length() == 0)
						return index + keys[i].length();
					pos = index + keys[i].length();
				} else
 ...
 public static String fullToHalfCorner(String input) {
	...
	public static String encodeValue(String value) {
		try {
			String newValue = Base64.encodeToString(value.getBytes("UTF-8"), Base64.NO_WRAP | Base64.URL_SAFE).replace('=', '*');
	...
	/**
	 * 操作数据库，写SQL语句时，对含有单引号的字符串进行处理，将一个单引号替换为两个单引号，达到转义的效果
	 * @param key
	 * @return new key
	 */
	public static String trimSingleQuotationMark(String key) {
		return key.replace("'", "''");
	}
	...
	public static Bitmap decodeFromBase64(String base64Image) {
		if("".equals(base64Image)||base64Image==null){
			return null;
		}
